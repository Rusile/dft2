...
def process_image(image):
    ...
    if len(contours) > 0:
        ...
        object_area = w * h
        frame_area = im_w * im_h
        object_percent = round((object_area / frame_area) * 100, 2)
        cv2.putText(image, str(object_percent) + '%', (50, 50), cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(0, 0, 255))
